...
public class P1011 {
    ...
    /*
        ...
        k광년을 이동하였을 때는 k-1 , k 혹은 k+1 광년만을 다시 이동 (처음 이동거리는 -1, 0, 1이므로 1만큼 이동)
        y지점에 도착하기 바로 직전의 이동거리는 반드시 1광년
     */
    public static int calculate(final int start, final int end){
        // 이동한 횟수
        int moveCnt = 0;

        // from : start
        // to : end - 1 (마지막은 1광년 이동이므로 1을 뺀다)
        // 이동거리는 k-1, k, k+1 (k : 직전 이동거리)
        int now = start;    // 현재 위치
        int to = end-1;     // now == to 이면 종료

        int totalLength = end - start + 1;   // 실제 이동해야할 거리
        int length = 0; // 이동한 거리
        ...
        for (int i = 0; i < totalLength; i++) {
            ...
            if(sumLeft + sumRight >= totalLength){
                break;
            }
        ...
        System.out.println(startSb.toString() + " / " + endSb.toString());

        return moveCnt;
    }

    public static void main(String[] args) {
        System.out.println(calculate(0, 3));    // 3
        System.out.println(calculate(1, 5));    // 3
        System.out.println(calculate(45, 50));  // 4
    }
}
